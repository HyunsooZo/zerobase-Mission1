...
<body>
    ...
    <table>
        ...
        <tbody>
            <%
                GroupDBTable group = new GroupDBTable();
            ...
            %>
            <tr>
                <td><%=ar.get(0).replace("\"", "")%></td>
                <td><%=ar.get(1).replace("\"", "")%></td>
                <td><%=ar.get(2).replace("\"", "")%></td>
                <td><%=ar.get(3).replace("\"", "")%></td>
                <td><%=ar.get(4)==null?"":ar.get(4).replace("\"", "")%></td>
                <td class="center">
                    <a href="group-modify.jsp?param1=<%=ar.get(0).replace("\"", "")%>&param2=<%=ar.get(1).replace("\"", "")%>&param3=<%=ar.get(2).replace("\"", "")%>">수정</a>
...
<script>
    function moveToAdd() {
        window.location.assign("group-add.jsp")
    }
